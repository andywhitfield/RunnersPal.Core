...
public class UserRouteService(IRouteRepository routeRepository, IRunLogRepository runLogRepository)
    : IUserRouteService
{
    public async Task<(List<Models.Route> Routes, Dictionary<int, RunLog> RunsByRouteId)> GetUserRoutesAsync(UserAccount userAccount)
    {
        var userRoutes = await routeRepository.GetRoutesByUserAsync(userAccount);
